...
def create_products_store(store, limit=False):
    ...
    if store.store_type.name == "VTEX":
        ...
        response = vtex_store.get_product_ids_with_sku_ids(
        ...
        if vtex_product_ids:
            Product.objects.filter(
                ~Q(external_id__in=vtex_product_ids), store=store
            ).delete()
            vtex_product_ids = [
                vtex_product_ids[i : i + 5000]
                for i in range(0, len(vtex_product_ids), 5000)
            ]
                        ...
                        defaults={
                            ...
                            "department": Department.objects.filter(
                                external_id__contains=[
                                    f"{store.store_type.name}{store.name}{product_response.get('DepartmentId')}"
                                ],
                                stores=store,
                            ).last(),
                            "sub_category": Subcategory.objects.select_related(
                                "category"
                            )
                            .filter(
                                external_id=product_response.get("CategoryId"),
                                category__store=store,
                            )
                            .last(),
                            "category": Subcategory.objects.select_related("category")
                            .filter(
                                external_id=product_response.get("CategoryId"),
                                category__store=store,
                            )
                            ...
                            "brand": Brand.objects.filter(
                                external_id__contains=[
                                    f"{store.store_type.name}{store.name}{product_response.get('BrandId')}"
                                ],
                                stores=store,
                            ).last(),
                            ...
                            "description_short": product_response.get(
                                "DescriptionShort"
                            ),
                            "keywords": product_response.get("KeyWords"),
                            "title": product_response.get("Title"),
                            "is_active": product_response.get("IsActive"),
                            "meta_tag_description": product_response.get(
                                "MetaTagDescription"
                            ),
                            "show_without_stock": product_response.get(
                                "ShowWithoutStock"
                            ),
                            "raw_json": {**product_response},
                        },
                    )
                    for product_response in product_responses
        ...
        if vtex_sku_ids:
            Sku.objects.filter(
                ~Q(external_id__in=vtex_sku_ids), product__store=store
            ).delete()
            vtex_sku_ids = [
                vtex_sku_ids[i : i + 5000] for i in range(0, len(vtex_sku_ids), 5000)
            ]
            for vtex_sku_id_list in vtex_sku_ids:
                ...
                for sku_response in sku_responses:
                    if not isinstance(sku_response, dict):
                        continue
                    s = []
                    instance_product = Product.objects.filter(
                        external_id=sku_response.get("ProductId"), store=store
                    ).last()
                    ...
                    instance_sku, _is_new = Sku.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            "comercial_condition_id": sku_response.get(
                                "CommercialConditionId"
                            ),
                            "manufacter_code": sku_response.get("ManufacturerCode"),
                            "reference_stock_id": sku_response.get(
                    ...
                    for sku_specification in sku_specifications:
                        (
                            attribute_type_instance,
                            _,
                        ) = AttributeType.objects.get_or_create(
                            store=store, name=sku_specification.get("FieldName")
                        )
                        values = sku_specification.get("FieldValues")
                        if values:
                            s.append(sku_specification.get("FieldName"))
                        for value in values:
                    ...
                    categories = (
                        ...
                        if sku_response.get("ProductCategories")
                        else []
                    )
                    search = " ".join(
                        [
                    ...
                    [
                        Seller.objects.get_or_create(
                            store=store,
                            seller_id=sku_seller.get("SellerId"),
                            defaults={
                                "name": sku_seller.get("SellerId"),
                        ...
                        [
                            SkuSeller.objects.update_or_create(
                                ...
                                seller=Seller.objects.filter(
                                    store=store, seller_id=sku_seller.get("SellerId")
                                ).last(),
                                defaults={
                                    "is_active": sku_seller.get("IsActive"),
                                    "raw_json": {**sku_seller},
                ...
                for sku_price_response in sku_price_responses:
                    instance_sku = Sku.objects.filter(
                        external_id=sku_price_response.get("itemId"),
    ...
    Sku.objects.filter(product__store=store).update(
        search_vector=SearchVector("search")
    )
    needed()
