...
# utilities
from chatbot_commerce.utils.models import BaseAbstract, BaseRawAbstract, ChatbootModel
...
class Skus(BaseAbstract):
    ...
    # Filter data
    is_active = models.BooleanField("Active sku", default=False, null=True, blank=True)
    is_inventoried = models.BooleanField(default=False, null=True, blank=True)
    total_quantity = models.CharField(
        "Quantity sku", max_length=255, null=True, blank=True
    )
    ...
    is_transported = models.BooleanField(default=False, null=True, blank=True)
    is_kit = models.BooleanField("sku is kit", default=False, null=True, blank=True)
    ...
    # Extra data
    comercial_condition_id = models.CharField(max_length=100, null=True, blank=True)
    manufacter_code = models.CharField(max_length=100, null=True, blank=True)
    ref_id = models.CharField("Reference id", max_length=100, null=True, blank=True)
    packaged_height = models.CharField(max_length=50, null=True, blank=True)
    packaged_length = models.CharField(max_length=50, null=True, blank=True)
    packaged_width = models.CharField(max_length=50, null=True, blank=True)
    packaged_weight = models.CharField(
        "Packaged weight Kg", max_length=50, null=True, blank=True
    )
    reference_stock_id = models.BooleanField(default=False, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        array_sku_seller = list(
            self.sku_seller.values_list("seller__seller_id", flat=True)
        )
        if array_sku_seller:
        ...
        self.serializer_data = {
            ...
            "attributes": list(
                self.attributes.values(
                    "value", attribute_name=models.F("attribute_type__name")
                )
            ),
...
class Image(ChatbootModel):
    ...
    image_id = models.CharField(
        _("Image external id"), max_length=50, null=True, blank=True
    )

    # Relationship filter
    sku = models.ForeignKey(
        "products.Skus", related_name="image_set", on_delete=models.CASCADE
    )
...
class Price(BaseRawAbstract):
    ...
    # Relationship filter
    sku = models.ForeignKey(Skus, related_name="price_set", on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        self.serializer_data = {
            ...
            "fixed_prices": list(
                self.fixed_prices.values_list("serializer_data", flat=True)
            ),
        }
        return super().save(*args, **kwargs)
...
class FixedPrice(BaseRawAbstract):
    ...
    def save(self, *args, **kwargs):
        self.serializer_data = {
            "value": self.value,
            "date_ranges": list(
                self.date_ranges.values("date_time_from", "date_time_to")
            ),
        }
        return super().save(*args, **kwargs)
...
class DateRange(BaseRawAbstract):
    ...
    date_time_from = models.DateTimeField(
        _("From date time"), auto_now=False, auto_now_add=False
    )
    date_time_to = models.DateTimeField(
        _("To date time"), auto_now=False, auto_now_add=False
    )
